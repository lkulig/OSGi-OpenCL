...
public class FlowExecutor {
		...
		if (statements != null) {
			for (CommonTree statement : statements) {
				...
				case GDTSTaskParser.COMMAND_CALL:
					...
					break;
				default:
					SimpleLogger.log("Uninmplemented flow for "
							+ statement.getToken().getText() + " token!");
				}
			}
		}
	}
	...
	private void assignStatement(CommonTree statement) {
		...
		case GDTSTaskParser.INCREMENT:
			helper.commandCall("INCREMENT", null, "var",
					varRef((CommonTree) statement.getChild(0)));
			break;
		case GDTSTaskParser.DECREMENT:
			helper.commandCall("DECREMENT", null, "var",
					varRef((CommonTree) statement.getChild(0)));
			break;
		default:
			varRef((CommonTree) statement.getChild(0)).setValue(
					expr((CommonTree) statement.getChild(1)).duplicate());
		}
	}
		...
		case GDTSTaskParser.ARRAY_REF:
			...
			for (int i = 1; i < node.getChildCount(); i++) {
				...
				if (indexNode.getToken().getType() == GDTSTaskParser.INDEX_BY_VAR) {
					var = ((VArray)var)
							.getElementAt((VInteger) varRef((CommonTree) indexNode
									.getChild(0)));
				}
			}
			return var;
	...
	private void commandCall(CommonTree statement) {
		...
		Map<String, Variable> params = commandParams(statement.getChildren()
				.subList(commandIdIndex + 1, statement.getChildren().size()));
	...
	private Map<String, Variable> commandParams(List<CommonTree> nodeList) {
		...
		for (CommonTree node : nodeList) {
			...
			params.put(node.getChild(0).getText(),
					expr((CommonTree) node.getChild(1)));
		}
	...
	private void whileLoop(CommonTree statement) {
		while (booleanExpr((CommonTree) statement.getChild(0))) {
			FlowExecutor executor = new FlowExecutor(
					(CommonTree) statement.getChild(1), this.helper);
			executor.execute();
		}
	}
	...
	private void forLoop(CommonTree statement) {
		...
		if (((CommonTree) statement.getChild(0)).getToken().getType() == GDTSTaskParser.FOR_INIT) {
			...
			assignStatement((CommonTree) ((CommonTree) statement.getChild(0))
					.getChild(0));
			conditionIndex = 1;
		}
		while (booleanExpr((CommonTree) statement.getChild(conditionIndex))) {
			FlowExecutor executor = new FlowExecutor(
					(CommonTree) statement.getChild(conditionIndex + 2),
					this.helper);
			executor.execute();
			assignStatement((CommonTree) ((CommonTree) statement
					.getChild(conditionIndex + 1)).getChild(0));
		}
	}
	...
	private boolean booleanExpr(CommonTree node) {
		return helper.evaluateBoolean(expr((CommonTree) node.getChild(0)),
				expr((CommonTree) node.getChild(2)),
