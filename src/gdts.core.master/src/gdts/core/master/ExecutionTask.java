...
class ExecutionTask implements Callable<Map<String, Variable>>, Executor {
	...
	public void commandCall(String id, ModuleDescr module,
		...
		if (module == null) {
			...
			try {
				master.repoService.getModuleService(module).execute(id, params);
	...
	public void systemCommandCall(String id, Map<String, Variable> params) {
		if (MasterCommands.isMasterCommand(id)) {
			if (id.equals(MasterCommands.Command.START_SLAVETASK.toString())) {
				String slaveTaskId = ((VString) params.get("slaveTask"))
						.toString();
				params.remove("slaveTask");
				GDTSNetId netId = ((VSlave) params.get("slave")).getNetId();
				params.remove("slave");
				Thread slaveTaskThread = startSlaveTask(netId, taskDescr.getId(),
						taskDescr.getVersion(), slaveTaskId, params);
				VSlaveTask slaveTask = (VSlaveTask) params.get("slaveTaskObj");
				slaveTask.createUniqueId();
				slaveTaskThreads.put((VSlaveTask) slaveTask.duplicate(),
						slaveTaskThread);
			...
			master.systemModule.execute(id, params);
		}
	}

	private Thread startSlaveTask(GDTSNetId slave, String taskId,
			Version version, String slaveTaskId, Map<String, Variable> params) {

		TaskDescriptor taskDescr = new TaskDescriptor(taskId, version);
		Thread thread = new Thread(new SlaveTaskRunnable(slave, taskDescr,
				slaveTaskId, params, params.get("result")));
		thread.start();
		return thread;
	}
	...
	class SlaveTaskRunnable implements Runnable, CommParty {
		...
		public SlaveTaskRunnable(GDTSNetId slave, TaskDescriptor taskDescr,
		...
		public void run() {
			...
			Message msg = new MasterStartSlaveTaskMessage(taskDescr,
					slaveTaskId, params);
...
//						+ ((SlaveTaskResultsMessage) msg).getData());
				...
				try {
					master.commService.sendTo(this, envelope.getSender(),
							((PingMessage) msg).messageOK());
